          ...
          <div class="book-header-info flex-grow-1">
            ...
            <div class="mb-6" v-if="!friendId">
              ...
              <div class="d-flex align-center">
                ...
                ></v-rating>
                <span class="ms-3 text-subtitle-1" v-if="bookRating > 0"
                  >{{ bookRating }} de 5 estrelas</span
                >
              </div>
            </div>
            ...
            <div class="d-flex flex-wrap gap-4">
              <template v-if="!friendId">
                <v-btn color="primary" variant="tonal" class="mr-2" prepend-icon="mdi-heart">
                  Favoritar
                </v-btn>
                <v-btn
                  color="secondary"
            ...
            <v-card class="mb-4 info-card pa-4" variant="outlined">
              ...
              <div class="mb-4">
                ...
                <div v-else class="d-flex align-center">
                  <v-icon :color="getStatusColor(selectedBook.status)" class="mr-2">{{
              ...
              <div v-if="selectedBook.status !== 0" class="reading-date mb-4">
                ...
                <template v-if="selectedBook.status === 2">
                  <p class="font-weight-medium mb-1">
                    {{ friendId ? `${friendName} começou a ler em` : 'Começou a ler em' }}
                  </p>
                  <BaseTextField
                    v-if="!friendId"
                ...
                <template v-else-if="selectedBook.status === 1">
                  ...
                  <p class="font-weight-medium mb-1">
                    {{ friendId ? `${friendName} concluiu a leitura em` : 'Leitura concluída em' }}
                  </p>
                  <BaseTextField
                    v-if="!friendId"
            ...
            <div v-if="showQuotesSection || friendId">
              ...
              <div class="quotes-list">
                  ...
                  <v-card-text>
                    ...
                    <template v-if="editingIndex === getPaginationIndex(index) && !friendId">
                      ...
                      <div class="d-flex gap-2 justify-end">
                        <v-btn
                          @click="saveQuoteEdit(getPaginationIndex(index))"
                          color="success"
                          size="small"
                          class="mr-2"
                          variant="tonal"
                          >Salvar</v-btn
                        >
                        <v-btn @click="cancelEdit" color="error" size="small" variant="tonal"
                          >Cancelar</v-btn
                        >
                      </div>
                    </template>
                    ...
                    <template v-else>
                      <div class="quote-text">
                        ...
                        <span v-if="quote.page" class="text-caption page-reference">
                          (Página {{ quote.page }})
                        </span>
                      </div>
                ...
                <p v-if="!filteredQuotes.length" class="text-center pa-4">
                  {{
                    quoteSearchQuery
                      ? 'Nenhuma frase corresponde à sua pesquisa.'
                      : friendId
                        ? `${friendName} ainda não adicionou frases favoritas.`
                ...
                <!-- Paginação -->
                <div
                  v-if="filteredQuotes.length > quotesPerPage"
    ...
    <v-dialog v-model="confirmDeleteBook" max-width="500px" transition="dialog-bottom-transition">
      <v-card class="delete-dialog">
        ...
        <v-card-text class="pa-4">
          <p class="mb-2">
            Tem certeza que deseja remover <strong>{{ selectedBook?.title }}</strong> da sua
            estante?
          </p>
    ...
    <v-dialog v-model="showQuoteDialog" max-width="500px">
      <v-card class="pa-4">
        ...
        <v-card-actions class="justify-end">
          <v-btn color="default" variant="text" @click="showQuoteDialog = false"> Cancelar </v-btn>
          <v-btn
            color="primary"
            variant="elevated"
...
onMounted(async () => {
  ...
  if (friendIdFromQuery) {
    ...
    try {
      ...
      if (
        selectedBook.value &&
        (selectedBook.value.status === undefined || selectedBook.value.status === null)
      ) {
...
// Substitua a função inteira no seu <script setup>

// Substitua esta função no seu <script setup>

// Substitua esta função no seu <script setup>

// Substitua esta função no seu <script setup>

// Em BookDetailsView.vue

const updateReadingStatus = async (newStatus: number) => {
  ...
  try {
    ...
    // Lógica para adicionar datas se necessário
    if (newStatus === 2 && !selectedBook.value.dataInicioLeitura) {
      updateData.started_reading_at = new Date().toISOString()
    } else if (newStatus === 1 && !selectedBook.value.dataFinalLeitura) {
    ...
    // CHAMA A FUNÇÃO GENÉRICA E CORRETA DA STORE
    await bookshelfStore.updateBook(bookId, updateData)

    // Como a 'updateBook' na store já atualiza o estado local,
    // a UI deve reagir corretamente.
...
const saveQuoteEdit = async (index: number) => {
  ...
  try {
    ...
    await bookshelfStore.editPhase(quoteId, {
...
const onRecommendationSent = (count: number) => {
  showNotification(
    `Livro recomendado para ${count} ${count === 1 ? 'amigo' : 'amigos'} com sucesso!`,
  )
}
...
@keyframes glowing {
  from {
    box-shadow: 0 10px 20px rgba(var(--v-theme-warning), 0.3);
  }
  to {
    box-shadow: 0 10px 20px rgba(var(--v-theme-warning), 0.7);
  }
}
